...
현재 거래 추가·목록·수정·삭제와 PRD-002 공통 실행 기반이 구현되어 있습니다.
카테고리 관리와 검색은 별도 기능 브랜치에 있으며, 현재 브랜치에는 월별 요약과
월 예산 설정·조회가 구현되어 있습니다. CSV 입출력은 이후 브랜치에서 구현합니다.
...
월 예산은 `budgets.jsonl`에 월당 한 건으로 저장됩니다. 같은 월의 예산을 다시
설정하면 다른 월을 유지하면서 해당 금액만 원자적으로 갱신합니다.
