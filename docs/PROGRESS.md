...
상태: PRD-002~004·007 기능 브랜치 구현 완료 / PRD-005~006 별도 브랜치 완료
...
- 현재 브랜치 `feature/csv-import-export`에서 PRD-007을 구현했다.

## PRD-007 CSV 가져오기·내보내기

상태: 구현 완료

### 완료된 작업

- 필수·선택·중복·알 수 없는 헤더와 행 열 수, CSV 구문 검증
- 날짜·타입·금액·등록 카테고리 검증과 선택 필드 기본값
- 임시 JSONL과 원자적 교체를 이용한 전체 성공 또는 전체 실패 가져오기
- 기존 최대 시퀀스 이후의 새 ID 발급과 반복 가져오기
- 월 또는 포함 날짜 범위 내보내기와 등록 역순 스트리밍
- 기존 출력 거부, 결과 없는 헤더 CSV, 한글·인용·줄바꿈 왕복
- CSV 경로·행 번호·원인·힌트와 실패 건수 오류 출력

### 검증 결과

- `uv run python -m unittest discover -v`: 72개 통과
- `uv run ruff format .`: 통과
- `uv run ruff check .`: 통과

### 다음 작업

1. CSV 브랜치를 커밋하고 원격 저장소에 푸시한다.
2. 월별 요약·예산과 CSV 기능을 `develop`에 통합한다.
3. 통합된 전체 테스트와 실제 CLI 흐름을 검증한다.
