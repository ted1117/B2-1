...
상태: PRD-002~006 기능 브랜치 구현 완료 / PRD-007 구현 미착수
...
- 현재 브랜치 `feature/monthly-budget`에서 PRD-006을 구현했다.

## PRD-006 월 예산

상태: 구현 완료

- 월별 예산 저장과 같은 월 원자적 갱신
- `summary`의 예산 금액, 소수점 한 자리 사용률, 실제 금액 기준 초과 경고
- 예산이 없는 달과 거래가 없는 달 처리
- 저장된 월·금액의 타입과 달력 유효성 검증
- 전체 테스트 57개, Ruff 포맷·검사 통과

### 다음 작업

1. [PRD-007](PRD-007.md)의 CSV 가져오기·내보내기를 구현한다.
2. 모든 기능 브랜치를 `develop`에 통합한다.
